<template>
  <div class="matrix">
    <template>
      <div
        v-for="(item, index) in matrixCollections"
        :key="item.collection + '-' + item.collectionID"
        class="item-matrix"
      >
        <div class="row">
          <div class="column">
...
export default {
  ...
  computed: {
    collectionsInput() {
      return _.map(this.matrixCollections, item => {
        return {
          id: item.matrixID,
    ...
    selectOptions() {
      ...
      return _.mapValues(_.keyBy(this.items, "collection"), item => this.render(item));
    },
    selectOptionsCollection() {
      return index => {
        ...
        return _.mapValues(_.keyBy(options, "id"), item => this.renderCollection(item));
      };
    },
    valueCollection() {
  ...
  watch: {
    CollectionsSave() {
      _.forEach(this.matrixCollections, (item, index) => {
  ...
  created() {
    ...
    this.$store.watch(
      ...
      function({ savedItem }) {
        ...
        _.forEach(_this.collectionsDeleted, item => {
          _this.$api.deleteItem("ene_matrix", item.matrixID);
        });

        _.forEach(_this.matrixCollections, item => {
  ...
  methods: {
    ...
    fetchMatrix() {
      ...
      this.$api.getItems("ene_matrix", params).then(({ data: items }) => {
        this.CollectionsSave = _.map(items, item => ({
          matrixID: item.id,
    ...
    fetchItems() {
      ...
      this.$api
        ...
        .then(({ data: items }) => {
          ...
          this.items = _.filter(newItems, function(item) {
